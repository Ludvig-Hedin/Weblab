...
> - **What's temporarily disabled** on Vercel until snapshot-based fork lands: `project.fork`, `branch.fork`, and `publish` (clear errors surfaced). Tracked as `TODO(sandbox-fork)` and `TODO(publish-vercel)` — see [docs/notes/2026-05-13-vercel-sandbox-provider.md](docs/notes/2026-05-13-vercel-sandbox-provider.md). Also disabled pending Convex port: prompt-based project create (hero AI input → toast `UNAVAILABLE_MESSAGE`) and GitHub-template imports — `TODO(sandbox-port)` in [apps/web/client/src/components/store/create/manager.ts](apps/web/client/src/components/store/create/manager.ts). Working create paths: "Start blank" CTA (hero + dashboard) → `api.projectActions.createBlank`.
