 ...
 */
const Feature43 = ({ eyebrow, heading, description, items, button, className }: Feature43Props) => {
    return (
        <section className={cn(className)}>
            <div className="mx-auto w-full max-w-6xl px-4 py-24 sm:px-6 md:px-8 md:py-32">
                ...
                <div className="grid grid-cols-1 gap-x-12 gap-y-14 md:grid-cols-2 lg:grid-cols-3">
                    {items.map((item, i) => (
                        <Reveal key={i} delay={0.1 + i * 0.05} y={16} className="flex flex-col">
                            ...
                            <h3 className="text-foreground-primary text-regularPlus mb-2">
                                {item.title}
                            </h3>
                            ...
                            {item.href ? (
                                <Link
                                    href={item.href}
                                    className="text-foreground-tertiary hover:text-foreground-primary text-small mt-4 inline-flex items-center gap-1 transition-colors"
                                >
                                    Learn more
