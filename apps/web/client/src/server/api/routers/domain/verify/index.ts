...
export const verificationRouter = createTRPCRouter({
    ...
    remove: protectedProcedure
        ...
        .mutation(async ({ ctx, input }) => {
            const existing = await ctx.db.query.customDomainVerification.findFirst({
    ...
    verifyOwnedDomain: protectedProcedure
        .input(
            z.object({
                fullDomain: z.string(),
                projectId: z.string(),
        ...
        .mutation(
            async ({
                ...
                const { customDomain } = await getCustomDomain(ctx.db, input.fullDomain);
                ...
                const verifiedDomain = await verifyFreestyleDomainWithCustomDomain(
                ...
                if (!verifiedDomain) {
                    return {
                        success: false,
                        failureReason:
                            'Failed to verify domain with Freestyle hosting provider. Please contact support as this is likely an issue with Freestyle.',
                    };
                }

                const [projectCustomDomain] = await ctx.db
                    .insert(projectCustomDomains)
                    .values({
                        projectId: input.projectId,
