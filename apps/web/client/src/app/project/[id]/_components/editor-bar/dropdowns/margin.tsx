...
export const Margin = observer(() => {
    ...
    const areAllMarginsEqual = useMemo((): boolean => {
        ...
        boxState.marginTop.num,
        boxState.marginRight.num,
        boxState.marginBottom.num,
        boxState.marginLeft.num,
    ]);
    ...
    const getMarginIcon = () => {
        ...
        const allSame =
            nonZeroValues.length === 4 && nonZeroValues.every((val) => val === nonZeroValues[0]);
        if (allSame) {
            return Icons.MarginFull;
        }

        // Create a pattern string for active sides in consistent order (T-R-B-L)
        const activeSides = SIDE_ORDER.filter((side) => margins[side] > 0)
    ...
    const getMarginDisplay = () => {
        ...
        if (nonZeroValues.length > 0 && nonZeroValues.every((val) => val === nonZeroValues[0])) {
            ...
            return boxState.margin.unit === 'px'
                ? `${nonZeroValues[0]}`
