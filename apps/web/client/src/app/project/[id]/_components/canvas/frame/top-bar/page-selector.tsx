            ...
            for (const page of pages) {
                ...
                items.push(
                    <DropdownMenuItem
                        ...
                        className={cn('cursor-pointer', isCurrentPage && 'bg-accent')}
                    >
                        <div
                            className="flex w-full items-center"
                            style={{ paddingLeft: `${depth * 16}px` }}
                        >
                            ...
                            <span className="truncate">{page.name}</span>
                            {isCurrentPage && <Icons.Check className="ml-auto h-3 w-3" />}
                        </div>
                    </DropdownMenuItem>,
                );
            ...
            return [
                {
                    ...
                    kind: 'page',
                    slug:
                        inferredCurrentPage.path === '/'
                            ? ''
                            : (inferredCurrentPage.path.split('/').filter(Boolean).pop() ?? ''),
            ...
            >
                ...
                >
                    <DropdownMenuTrigger asChild>
                        ...
                        >
                            {showIcon && <Icons.File className="mr-2 h-4 w-4" />}
                            <span className="max-w-24 truncate">{displayCurrentPage.name}</span>
                        </Button>
                    </DropdownMenuTrigger>
                </HoverOnlyTooltip>
