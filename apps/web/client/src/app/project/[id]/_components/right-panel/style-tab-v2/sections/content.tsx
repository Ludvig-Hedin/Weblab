...
function FilterEditor({
    ...
    useEffect(() => {
        ...
        setFilterIds(filters.map((_, i) => `fid-${i}`));
        idCounterRef.current = filters.length;
    ...
    const add = () => {
        ...
        setFilterIds((prev) => [...prev, `fid-${idCounterRef.current++}`]);
    };
            ...
            {localFilters.length === 0 ? (
                ...
                <ul className="space-y-1.5">
                    {localFilters.map((clause, idx) => {
                        ...
                        return (
                            <li key={filterIds[idx] ?? idx} className="flex items-center gap-1.5">
                                <Select
