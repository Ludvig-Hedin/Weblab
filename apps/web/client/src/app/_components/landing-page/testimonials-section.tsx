...
function TestimonialCard({
    ...
    const cardContent = (
        <div
            className={`bg-background-weblab border-foreground-secondary/10 flex min-h-[160px] flex-col justify-between rounded-xl border p-6 duration-200 select-none ${href ? 'hover:bg-background-weblab hover:border-foreground-secondary/20 cursor-pointer transition-colors duration-50' : ''}`}
        >
            <div className="text-foreground-primary text-regular mb-6">{text}</div>
            <div className="mt-auto flex items-center gap-3">
                {/* Profile picture */}
                <div className="flex h-10 min-h-10 w-10 min-w-10 items-center justify-center overflow-hidden rounded-full">
                    {profileImage ? (
                        <img
                            src={profileImage}
                            alt={`${name} profile`}
