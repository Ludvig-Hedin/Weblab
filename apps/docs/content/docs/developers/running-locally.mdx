...
### 1. Clone the repository
   ...
   ```bash
   git clone https://github.com/Ludvig-Hedin/Weblab.git
   cd onlook
   bun install
   ```
...
If a code change requires migrations, config, env values, or setup commands,
the agent or contributor making the change must either run the required local
command or clearly document the exact manual step that remains.
...
Before handing off changes, run the narrowest validation that proves the touched
area still works:

- `bun typecheck` for web-client TypeScript changes
- `bun lint` or targeted ESLint for lint-sensitive code
- `bun test` or targeted package tests for tested logic
- `bun db:push` when schema or selected DB columns changed
- `git diff --check` for documentation-only changes

Do not leave the app with known avoidable runtime, build, type, lint,
migration, or config errors.
