...
Recent local databases may also need the latest Supabase migrations from
`apps/backend/supabase/migrations`, including runtime-mode metadata and
`user_settings` preference columns. If you change Drizzle schema or selected
columns, validate the local database with `bun db:push` before handoff.
