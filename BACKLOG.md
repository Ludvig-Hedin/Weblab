...
- **Open findings (ranked; verify line refs before fixing):**
...
- **Findings (ranked; verify each line ref before fixing):**
  1. **BLOCKER — `[object Object]` written on prop-reset in HTML projects** (`packages/parser/src/pipelines/html/index.ts:~477`): the `__remove: true` sentinel that the JSX path understands is coerced via `String(raw)` in the HTML pipeline, so "reset prop to default" writes the literal `"[object Object]"` into the HTML attribute instead of removing it — corrupts source HTML, compounding per reset. Fix: check `raw?.__remove === true` → `removeAttribute` + `continue` before the `String(raw)` coercion. **Likely unit-testable in the parser — strong next-fix candidate.** (Relates to the known-open "HTML edit routing + prop undo" gap, agent-memory `project_component_system`.)
