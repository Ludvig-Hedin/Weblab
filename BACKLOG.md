...
- **Where / Symptom / Next step:** each item also has a matching `TODO(bug-hunt)` comment in code:
  ...
  4. **Interrupted-stream recovery regenerates the wrong turn** — `use-chat/index.tsx` auto-regen + `api/chat/route.ts` abort path: the interrupted user message is never persisted, so reload regenerates the *previous* question and the new one is lost. Fix: persist pending user content alongside the inflight flag. `#bug`
  ...
  7. **Expired-pending re-invite blocker (project invites)** — `convex/projectInvitations.ts` `_validateAndInsert` still has the bug fixed in `workspaces.inviteCreate` this session (expired `pending` rows block re-invites). Port the same pattern. `#bug`
