...
- 🔍 **STILL UNVERIFIED (0 verifiers ran — throttled):** `canvas/frame/index.tsx:429` — sandbox auto-restore may fire 3× concurrent `Sandbox.create` on a single 410 (orphaned VMs + DB write race). `#sandbox`
- 🔍 **STILL UNVERIFIED + prior dispute:** `screenshot/index.tsx:29` — navigate-away screenshot capture possibly lost to the 10s debounce; one verifier already disputed the "never refresh" conclusion. Lowest-confidence lead. `#thumbnail`

> NOTE for next pass: re-verify the 2 remaining leads (`frame/index.tsx:429` auto-restore, `screenshot/index.tsx:29`) and decide the `/create`-`/fix` label fix. Server throttling has been active across iter-19 — keep batches small + back off on rate-limit.
