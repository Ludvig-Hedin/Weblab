...
1. Update the providers in [`packages/ai/src/chat/providers.ts`](https://github.com/Ludvig-Hedin/Weblab/blob/main/packages/ai/src/chat/providers.ts). We already support Anthropic and OpenRouter as examples. You can follow the same format to add a new provider.
2. Update the usages by searching for [`initModel`](https://github.com/search?q=repo%3ALudvig-Hedin%2FWeblab+%22await+initModel%22&type=code)
