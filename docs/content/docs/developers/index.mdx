...
Weblab is structured as a Bun workspace monorepo with several interconnected apps and packages, primarily built using Next.js App Router, React, TypeScript, TailwindCSS, tRPC, Supabase, and Drizzle.

For agent-oriented technical context, see the optional context pack in
`docs/agent-context/`. It summarizes current progress, repository structure,
development setup, editor architecture, data/API boundaries, and product/design
expectations without replacing the public docs.
...
When contributing to this repository, please first discuss the change you wish to make via [issues](https://github.com/Ludvig-Hedin/Weblab/issues),
...
Please note we have a [code of conduct](https://github.com/Ludvig-Hedin/Weblab/blob/main/CODE_OF_CONDUCT.md), please follow it in all your interactions with the project.

### Running the project locally
You can build the project from source using instructions [here](/developers/running-locally).

Key local setup steps are:
...
Run `bun db:push` after database schema changes. Do not run `bun db:gen`; it is
