...
| F-785 | `#ai` `#design` | Component registry + anti-slop system prompt | [component-registry/](component-registry/) + [prompt/constants/{design,components,slop-checklist}.ts](packages/ai/src/prompt/constants/) | Constrains the builder agent to a curated component catalog (`COMPONENT_REGISTRY` in `@weblab/constants`, mirrored from `component-registry/manifest.json` — 16 shadcn/ui new-york + 5 Watermelon UI) and ONE design-token palette (`component-registry/theme/tokens.css`, OKLCH). Three blocks injected into the cached system prefix via both `cache-blocks.ts` (stable) and `provider.ts`: `<design-system>` (design mandate), `<component-registry>` (default Next.js+React+shadcn stack, use-tokens-never-invent-color/font, install via `bunx --bun shadcn add "<url>"`, match existing project), `<anti-slop-checklist>` (pre-return gate, last). Override order: attached reference > existing project's tokens/fonts/stack > defaults; forbidden patterns are defaults, not censorship. Folder extendable via `scripts/fetch-components.mjs`. Full field guide: [docs/agent-context/ai-slop-field-guide.md](docs/agent-context/ai-slop-field-guide.md) | Every AI build of a new site or new section — keeps output on-brand, consistent, non-slop |
