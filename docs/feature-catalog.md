...
| F-121 | `#auth-gated` | `/projects/new` | [src/app/projects/new/page.tsx](apps/web/client/src/app/projects/new/page.tsx) | AI prompt + template chips + a compact row of icon buttons w/ tooltips (`project-chooser-cards.tsx`): Start blank · Open folder (desktop) / Upload folder (web) · Clone a site · GitHub. "Start blank" → cleaner create dialog (`framework-select-dialog.tsx`) with a Cloud/Local destination toggle — Cloud = all ready frameworks; Local (desktop only) = Next.js / Static HTML scaffolded to disk via `useOpenLocalProject.createLocalBlank` | Create project |
...
| F-786 | `#desktop` `#auth-gated` | Folder drag-and-drop | [components/desktop/desktop-folder-drop.tsx](apps/web/client/src/components/desktop/desktop-folder-drop.tsx) + [apps/desktop/main.js](apps/desktop/main.js) + [apps/desktop/preload.js](apps/desktop/preload.js) | Drag a folder into the window (full-window overlay) or drop it on the dock icon / "Open With Weblab" (macOS `open-file` → `weblab:open-folder` IPC, queued until `weblab:renderer-ready`) → `useOpenLocalProject.openLocalFolderAtPath` (empty → Static HTML scaffold, else open as-is). Path resolved via `getPathForDroppedFile` (`webUtils.getPathForFile`; `File.path` removed in Electron 32+); `mac.extendInfo.CFBundleDocumentTypes` registers the folder handler | Open a local folder by dragging it in |
...
| F-753 | `#internal` | Service worker | [public/sw.js](apps/web/client/public/sw.js), F-057 | PWA app shell, offline fallback, runtime asset cache; `/_next/static/chunks/*` network-first to avoid stale deploy hydration |
...
| F-793 | `#ai` `#convex` | [convex/wireframeEmit.ts](apps/web/client/convex/wireframeEmit.ts) + [emit-button.tsx](apps/web/client/src/app/project/%5Bid%5D/wireframe/_components/emit-button.tsx) | Emit a REAL shadcn Next.js project: pages + real block sources + vendored standard shadcn primitives + `cn` + radix/lucide deps + style-guide globals, then `bun install`. **Cloud** = `emitToCloud` action (Vercel Sandbox, mirrors `createFromFigma`); **local** = desktop NodeFs bridge (`window.weblabNative.localfs`/`localdev`) writing into the project root. Opens the editor. |
