...
| T-121a | F-121 | M (desktop) | Start blank → Local → Next.js, pick empty folder | Scaffolds Next.js to disk; `npm install` + `next dev` boot on a free port (not :3000); editor renders. Repeat Local → Static HTML | `[ ]` |
| T-786 | F-786 | M (desktop) | Drag a folder into the window; drop a folder on the dock icon | Overlay shows on drag; empty folder → Static HTML scaffold + open; non-empty → opens as-is; dropping a file (not a folder) → "Drop a folder, not a file" toast; dock-drop on a cold launch opens after sign-in | `[ ]` |
