...
# shadcn — design system + component catalog

Use this skill whenever you build a page or add a section. It gives you (1) the
shadcn-style design foundations to follow and (2) the full catalog of components
and blocks you may install, with how to install each.

## Use the catalog, don't reinvent

- **Default stack:** Next.js (App Router) + React + TypeScript + Tailwind + shadcn/ui.
- **Install, don't hand-roll.** Pick the closest item from the catalog below and
  install it into the user project with its source's install command (patterns at
  the top of the catalog). Only hand-write a component when nothing in the catalog
  fits.
- **Prefer fewer dependencies.** Reach for shadcn/ui primitives and local pro
  blocks first; use shadcnblocks / Watermelon for richer sections.
- **Adapt, don't dump.** After installing a block, replace its demo copy, links,
  images, and sample data with the user's real content.

## Style with tokens — never invent colors or fonts

- Color and radius come ONLY from the project's design tokens (the shadcn CSS-var
  contract: `--background`, `--foreground`, `--primary`, `--muted`,
  `--border`, `--ring`, `--radius`, …). Style with token utilities
  (`bg-background`, `text-foreground`, `text-muted-foreground`, `border-border`,
  `ring-ring`, `bg-primary`). Never hardcode a hex/rgb or add an arbitrary color.
- One accent only (the teal `--ring` / `--brand-accent`), on focus/active/one CTA,
  under ~5% of the screen — never on body text or headings.
- Fonts come only from the approved set (Geist for Direction A; Fraunces + Geist
  for Direction B). Don't introduce a third family.

## Existing projects — match, never introduce

When editing or adding to an existing site, read its `globals.css` tokens and its
installed components first. Use that project's tokens, fonts, stack, and existing
components. Do NOT introduce a new color, font, or component library. A new
section must look like it was always part of the site.

## Design foundations (shadcn style)

- **Type scale:** 12/14/16/20/24/32; weights 400/500/600 (avoid heavy bold walls).
  Tabular numerals on data. Sentence case; no Title-Case buttons.
- **Spacing:** 4/8/12/16/24/32 rhythm. Airy between sections, tight within.
- **States:** every interactive element needs default, hover (2+ properties),
  `:focus-visible` ring, active/press, disabled, and (where relevant) loading and
  error. Keyboard-first; visible focus.
- **Flat:** depth from spacing, hairlines, and 1px borders — not shadow spam.
- **Accessibility:** WCAG 2.2 AA. Body contrast ≥ 4.5:1. Don't signal state with
  color alone.

## QA before returning

Run the anti-slop checklist (it's in your system prompt): committed direction,
tinted impure neutrals, one accent off the text, token-only color, no card/shadow
spam, real states, specific copy, unrecognizable-as-a-starter. Fix every miss.

---

## Catalog

The complete machine-readable catalog (with per-item descriptions) lives in the
Weblab repo at `component-registry/manifest.json` and `component-registry/CATALOG.md`.
The index below lists every available item by source and category — install any of
them by name with the matching pattern.
