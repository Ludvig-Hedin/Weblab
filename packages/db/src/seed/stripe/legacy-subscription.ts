...
export const seedLegacySubscriptions = async () => {
    ...
    const emails: string[] = readFileSync(
        path.join(__dirname, './subscriptions.csv'),
        'utf8',
    ).split('\n');
    ...
    for (const email of emails) {
        if (!email) continue;
        const { id: stripePromotionCodeId, code: stripePromotionCode } = await createCodeForCoupon(
            stripe,
            stripeCouponId,
            email,
        );
